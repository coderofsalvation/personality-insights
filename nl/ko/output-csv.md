...
CSV 결과의 정보는 입력이 시간소인이 있는 데이터를 표시하는지 여부 및 사용자가 원 점수와 소비 성향을 요청하는지 여부에 따라 다릅니다.
{: shortdesc}

그러나 JSON과는 달리, CSV 결과는 고정된 열 수로서 리턴됩니다. 결과의 첫 번째 행은 선택적 열 레이블로 구성되며,
...
이해하기](/docs/services/personality-insights/output.html) 및 [숫자 결과 해석](/docs/services/personality-insights/numeric.html)을 참조하십시오. 
...
<table>
  ...
  <tr>
    ...
    <td>
      이름 지정된 특질 또는 성향과 관련하여 텍스트 작성자의 정규화된 백분위수 점수.
    </td>
  </tr>
  <tr>
    <td>
      빅 파이브 성실성<br/>백분위수<br/>(7개 열)
    </td>
    <td>
  ...
  <tr>
    ...
    <td>
      이름 지정된 욕구와 관련하여 텍스트 작성자의 정규화된 백분위수 점수.
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      이름 지정된 가치와 관련하여 텍스트 작성자의 정규화된 백분위수 점수.
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      입력 텍스트에 시간소인이 있는 경우, 각 요일과 연관된 입력의 백분율.
      입력에 시간소인이 없는 경우, 백분율은 모두 <code>0.0</code>입니다.
    </td>
  </tr>
  <tr>
    <td>
      일중 시간<br/>백분율<br/>(24개 열)
    </td>
    <td>
      behavior_0000<br/><em> - </em><br/>behavior_2300
    </td>
    <td>
      입력 텍스트에 시간소인이 있는 경우, 각 일중 시간과 연관된 입력의 백분율.
      입력에 시간소인이 없는 경우, 백분율은 모두 <code>0.0</code>입니다.
    </td>
  </tr>
  <tr>
    <td>
      단어 개수 및<br/>언어<br/>(2개 열)
    </td>
    <td>
...
다음 열은 `raw_scores` 조회 매개변수를 `true`로 설정하여 원 점수를 요청하는 경우에만 존재합니다.
모든 경우, 열은 특질, 성향, 욕구 또는 가치에 대한 작성자의 원 점수를 제공하는 double 값입니다. 
